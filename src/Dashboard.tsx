...
import { Target, Users, CalendarDays, AlertTriangle, Gift, Settings, ChevronLeft, ChevronRight, BarChart3, CreditCard } from 'lucide-react';
  ...
  return (
    <div className="space-y-12 pb-24 max-w-[1700px] mx-auto">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-8 px-1">
        ...
        >
          <div className="flex items-center gap-4 mb-3">
             <Badge variant="outline" className="px-3 py-1 border-primary/20 bg-primary/5 text-primary text-[10px] font-black uppercase tracking-[0.2em] rounded-full animate-pulse-soft">
              Logic Stream: Active
            </Badge>
          ...
          <h2 className="text-7xl font-black tracking-tighter leading-none mb-4">
            Welcome, <span className="text-gradient-primary italic">{currentUser?.displayName?.split(' ')[0] || 'Commander'}</span>
          </h2>
          <p className="text-lg font-bold text-muted-foreground tracking-tight opacity-60 max-w-2xl leading-relaxed">
            Propelling Strike Boxing through high-fidelity data synchronization and strategic personnel logistics. Your fleet is operational.
          </p>
        </motion.div>
              ...
              <DialogContent className="glass-card border-zinc-200/50 dark:border-zinc-800/50 shadow-2xl rounded-3xl p-8 max-w-md">
                <DialogHeader>
                  <DialogTitle className="text-3xl font-black tracking-tighter uppercase leading-none mb-1">
                    Strategy <span className="text-primary italic">Parameters</span>
                  </DialogTitle>
                  <p className="text-xs font-bold text-muted-foreground opacity-50 uppercase tracking-widest">Adjust operational objectives</p>
                </DialogHeader>
                <div className="space-y-8 py-8">
                  <div className="space-y-3">
                    <Label className="font-black text-[10px] uppercase tracking-[0.3em] opacity-40">Monthly Revenue Objective (LE)</Label>
                  ...
                  <Button onClick={handleUpdateTarget} className="w-full h-16 text-xs font-black uppercase tracking-[0.3em] bg-primary hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl shadow-primary/20 rounded-2xl">Deploy Strategy Update</Button>
                </div>
              </DialogContent>
            </Dialog>
        </motion.div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
        <KPICard 
          title="Gross Strategy" 
          value={`${salesTarget.currentAmount.toLocaleString()} LE`}
          icon={CreditCard}
          trend={{ value: 12, isPositive: true }}
          description={`${targetPercentage}% of objective`}
          color="rose-600"
        />
        <KPICard 
          title="Growth Engine" 
        ...
        <KPICard 
          ...
          description="Qualified lead to member"
          color="amber-600"
        />
        <KPICard 
          title="Risk Exposure" 
          ...
          description="Churn risk < 30 days"
          color="red-600"
        />
        <KPICard 
          ...
          icon={Gift}
          description="Celebration window (7d)"
          color="pink-600"
        />
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <Card className="glass-card shadow-2xl border-none overflow-hidden group">
          ...
          <CardHeader className="bg-zinc-100/50 dark:bg-zinc-900/50 border-b border-zinc-200/50 dark:border-zinc-800/50 pb-4 pt-6 px-8 flex flex-row items-center justify-between">
            ...
            <span className="text-[10px] font-black tabular-nums opacity-20">LVL 4 ALRT</span>
          </CardHeader>
              ...
              {(stats.negativeSessions.length > 0 || stats.noAttendance.length > 0 || stats.expiredList.length > 0) ? (
                <>
                  {stats.negativeSessions.length > 0 && (
                    <div className="space-y-4">
                      <h4 className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40 px-2 border-l-2 border-red-500/50">Deficit Protocols</h4>
                      <PaginatedList 
                  ...
                  {stats.noAttendance.length > 0 && (
                    <div className="space-y-4">
                      <h4 className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40 px-2 border-l-2 border-amber-500/50">Stagnation Alerts</h4>
                      <PaginatedList 
                  ...
                  {stats.expiredList.length > 0 && (
                    <div className="space-y-4">
                      <h4 className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40 px-2 border-l-2 border-zinc-500/50">Terminated Access</h4>
                      <PaginatedList 
                        ...
                        renderItem={(client: Client) => (
                          <div key={client.id} className="flex items-center justify-between p-4 border border-zinc-200/50 dark:border-zinc-800/50 bg-white/40 dark:bg-black/20 rounded-2xl opacity-60">
                            ...
                            <Badge variant="outline" className="font-black rounded-lg uppercase text-[10px] opacity-40">Offline</Badge>
                          </div>
                        )} 
                      />
                    </div>
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <h3 className="text-sm font-black uppercase tracking-widest mb-1 opacity-70">Efficiency Optimal</h3>
                    <p className="text-[10px] font-bold text-muted-foreground opacity-30 uppercase tracking-tight">All systems operational. No critical gaps detected.</p>
                </div>
              )}
            </div>
          </CardContent>
        ...
        <Card className="glass-card shadow-2xl border-none overflow-hidden group">
          ...
          <CardHeader className="bg-zinc-100/50 dark:bg-zinc-900/50 border-b border-zinc-200/50 dark:border-zinc-800/50 pb-4 pt-6 px-8 flex flex-row items-center justify-between">
            <CardTitle className="text-xs font-black uppercase tracking-[0.3em] text-blue-500/80 flex items-center gap-2">
               ...
               <span>Personnel Momentum</span>
            </CardTitle>
            <Badge variant="outline" className="font-black text-[9px] uppercase tracking-widest border-blue-500/20 text-blue-500">Sync</Badge>
          </CardHeader>
          <CardContent className="p-6">
            <div className="space-y-8">
              {(stats.upcomingVisits.length > 0 || stats.nearlyExpiredList.length > 0) ? (
                <>
                  {stats.upcomingVisits.length > 0 && (
                    <div className="space-y-4">
                      <h4 className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40 px-2 border-l-2 border-blue-500/50">Expected Activity</h4>
                      <PaginatedList 
                          ...
                          <div key={client.id} className="flex items-center justify-between p-4 border border-zinc-200/50 dark:border-zinc-800/50 bg-blue-500/5 rounded-2xl">
                            <div className="space-y-1">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-[9px] font-black text-blue-600/80 uppercase tracking-widest">ETA: {new Date(client.expectedVisitDate!).toLocaleDateString()}</span>
                              </div>
                            </div>
                          </div>
                        )} 
                  ...
                  {stats.nearlyExpiredList.length > 0 && (
                    <div className="space-y-4">
                      <h4 className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40 px-2 border-l-2 border-amber-500/50">Churn Risk Horizon</h4>
                      <PaginatedList 
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center opacity-30">
                    ...
                    <p className="text-xs font-black uppercase tracking-widest">Stagnant Momentum</p>
                </div>
              )}
            </div>
          </CardContent>
